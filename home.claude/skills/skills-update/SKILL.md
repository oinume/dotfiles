...
This skill is installed globally, so it can be invoked from any working directory.
Every command below must therefore use absolute paths derived from the dotfiles
repository root, never paths relative to the current directory.

## Workflow

1. Resolve the repository root and confirm it is this dotfiles repository. If the
   current directory is not inside it, use the checkout path explicitly instead of
   guessing:
...
- Never run a command with a bare relative `--dir`; always pass `"$ROOT/..."`.
